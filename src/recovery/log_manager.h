...
public:
    ...
    DeleteLogRecord(txn_id_t txn_id, RmRecord& old_record, Rid& rid, std::string table_name)
        : DeleteLogRecord() {
        log_tid_ = txn_id;
        old_record_ = RmRecord(old_record);
        rid_ = rid;
        log_tot_len_ += sizeof(int);
    ...
    void deserialize(const char* src) override {
        LogRecord::deserialize(src);
        old_record_.Deserialize(src + OFFSET_LOG_DATA);
        int offset = OFFSET_LOG_DATA + old_record_.size + sizeof(int);
        rid_ = *reinterpret_cast<const Rid*>(src + offset);
        offset += sizeof(Rid);
        table_name_size_ = *reinterpret_cast<const size_t*>(src + offset);
        offset += sizeof(size_t);
    ...
    RmRecord old_record_;   // before-image for undo
    Rid rid_;
    char* table_name_;
    size_t table_name_size_;
};
...
public:
    ...
    UpdateLogRecord(txn_id_t txn_id, RmRecord& old_record, RmRecord& new_record,
                    Rid& rid, std::string table_name) : UpdateLogRecord() {
        log_tid_ = txn_id;
        old_record_ = RmRecord(old_record);
        new_record_ = RmRecord(new_record);
        rid_ = rid;
        log_tot_len_ += sizeof(int);
        log_tot_len_ += old_record_.size;
        log_tot_len_ += sizeof(int);
    ...
    RmRecord old_record_;   // before-image for undo
    RmRecord new_record_;   // after-image for redo
    Rid rid_;
    char* table_name_;
    size_t table_name_size_;
};
