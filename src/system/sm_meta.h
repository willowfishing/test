...
struct TabMeta {
    ...
    /* DDL用：精确匹配索引列集合（create/drop index） */
    bool is_index(const std::vector<std::string>& col_names) const {
        if(col_names.empty()) return false;
    ...
    /* Planner用：最左前缀匹配（select/delete/update） */
    bool is_index_prefix(const std::vector<std::string>& col_names) const {
        if(col_names.empty()) return false;
        for(auto& index: indexes) {
    ...
    std::vector<IndexMeta>::iterator get_index_meta(const std::vector<std::string>& col_names) {
        if(col_names.empty()) throw IndexNotFoundError(name, col_names);
...
class DbMeta {
   ...
   public:
    std::string &name() { return name_; }   // 获取数据库名称
