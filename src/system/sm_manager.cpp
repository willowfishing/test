...
void SmManager::show_tables(Context* context) {
    std::fstream outfile;
    std::string output_path = db_.name() + "/output.txt";
...
void SmManager::show_index(const std::string& tab_name, Context* context) {
    ...
    for(auto &index : tab.indexes) {
        ...
        cols_str += ")";
        outfile << "| " << tab.name << " | unique | " << cols_str << " |\n";
    }
    outfile.close();
}
...
void SmManager::create_index(const std::string& tab_name, const std::vector<std::string>& col_names, Context* context) {
    ...
    for (auto &col_name : col_names) {
        ...
        col_metas.push_back(*it);
        col_tot_len += it->len;
    }
    // 唯一性检查：扫描表中已有数据，确认索引列组合无重复值
    {
        ...
        for (RmScan scan(fh); !scan.is_end(); scan.next()) {
            ...
            for (auto &col : col_metas) {
                key_str.append(rec->data + col.offset, col.len);
            }
            if (key_set.count(key_str)) {
    ...
    // 打开索引文件
    std::string ix_name = ix_manager_->get_index_name(tab_name, col_metas);
    ...
    for (RmScan scan(fh); !scan.is_end(); scan.next()) {
        ...
        ihs_.at(ix_name)->insert_entry(key, rid, context->txn_);
        delete[] key;
    }
    flush_meta();
}
...
void SmManager::drop_index(const std::string& tab_name, const std::vector<std::string>& col_names, Context* context) {
    ...
    std::vector<ColMeta> cols = idx_it->cols;
