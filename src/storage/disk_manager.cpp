...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    ...
    if (lseek(fd, pos, SEEK_SET) < 0) { throw UnixError(); }
    ssize_t bytes_written = write(fd, offset, num_bytes);
    if (bytes_written != num_bytes) { throw InternalError("DiskManager::write_page Error"); }
}
...
void DiskManager::read_page(int fd, page_id_t page_no, char *offset, int num_bytes) {
    ...
    if (bytes_read != num_bytes) { throw InternalError("DiskManager::read_page Error"); }
}
...
int DiskManager::get_file_fd(const std::string &file_name) {
    if (!path2fd_.count(file_name)) { return open_file(file_name); }
