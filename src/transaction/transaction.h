...
class Transaction {
   public:
    ...
    // SSI: read set tracking
    std::shared_ptr<std::unordered_set<Rid>> read_set_ = std::make_shared<std::unordered_set<Rid>>();
    inline void add_to_read_set(const Rid &rid) { read_set_->insert(rid); }
    inline bool in_read_set(const Rid &rid) { return read_set_->count(rid) > 0; }
    inline auto get_read_set() { return read_set_; }

    // SSI: predicate/table-level reads (for phantom detection)
    std::shared_ptr<std::unordered_set<std::string>> predicate_tables_ =
        std::make_shared<std::unordered_set<std::string>>();
    std::shared_ptr<std::unordered_set<std::string>> write_tables_ =
        std::make_shared<std::unordered_set<std::string>>();
    inline void add_predicate_read(const std::string &tab_name) { predicate_tables_->insert(tab_name); }
    inline auto get_predicate_reads() { return predicate_tables_; }
    inline void add_write_table(const std::string &tab_name) { write_tables_->insert(tab_name); }
    inline auto get_write_tables() { return write_tables_; }

    // SSI: rw-dependencies
    std::unordered_set<txn_id_t> rw_in_;   // txns that wrote something I read
    std::unordered_set<txn_id_t> rw_out_;  // txns that read something I wrote
    // Predicate-based rw deps (separate from write-path deps)
    std::unordered_set<txn_id_t> pred_rw_in_;
    std::unordered_set<txn_id_t> pred_rw_out_;
    bool ssi_should_abort_ = false;        // set by SSI check
    inline void add_rw_in(txn_id_t txn_id) { rw_in_.insert(txn_id); }
    inline void add_rw_out(txn_id_t txn_id) { rw_out_.insert(txn_id); }
    inline void add_pred_rw_in(txn_id_t txn_id) { pred_rw_in_.insert(txn_id); }
    inline void add_pred_rw_out(txn_id_t txn_id) { pred_rw_out_.insert(txn_id); }
