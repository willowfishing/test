...
struct RmPageHandle {
    ...
    char *slots;                // page->data的第三部分，存储表的记录，指针指向首地址，每个slot的长度为file_hdr->record_size + RM_TUPLE_META_SIZE
    ...
    // 每个slot的完整大小（TupleMeta + 记录数据）
    int slot_stride() const { return file_hdr->record_size + RM_TUPLE_META_SIZE; }

    // 返回指定slot_no的slot存储首地址
    char* get_slot(int slot_no) const {
        return slots + slot_no * slot_stride();
    }
    ...
    // 返回记录数据指针（跳过TupleMeta）
    char* get_record_data(int slot_no) const {
        return get_slot(slot_no) + RM_TUPLE_META_SIZE;
    }
};
...
class RmFileHandle {      
   ...
   public:
    ...
    // Read both TupleMeta and record data in a single page visit
    std::pair<TupleMeta, std::unique_ptr<RmRecord>> get_meta_and_record(const Rid &rid) const {
        RmPageHandle page_handle = fetch_page_handle(rid.page_no);
    ...
    // TupleMeta access
    TupleMeta get_meta(const Rid &rid) const {
        RmPageHandle page_handle = fetch_page_handle(rid.page_no);
        TupleMeta meta = *page_handle.get_meta(rid.slot_no);
        buffer_pool_manager_->unpin_page(page_handle.page->get_page_id(), false);
        return meta;
    ...
    void set_meta(const Rid &rid, const TupleMeta &meta) {
        RmPageHandle page_handle = fetch_page_handle(rid.page_no);
