...
class RmFileHandle {
   ...
   public:
    RmFileHandle(DiskManager *disk_manager, BufferPoolManager *buffer_pool_manager, int fd)
        ...
        // A crash may occur after a new data page reaches disk but before the
        // in-memory file header is persisted. Never reuse such a physical page.
        const int physical_size = disk_manager_->get_file_size(disk_manager_->get_file_name(fd));
        const int physical_pages = std::max(1, (physical_size + PAGE_SIZE - 1) / PAGE_SIZE);
        file_hdr_.num_pages = std::max(file_hdr_.num_pages, physical_pages);
    ...
    void repair_free_page_list(const std::vector<int> &affected_pages);
