...
QCmd parse_qcmd(const char *raw_sql) {
...
bool ExecuteQCmd(QCmd command, txn_id_t *txn_id, Context *context,
                        IsolationLevel *session_isolation) {
    if (command == QCmd::NoMatch) {
        return false;
    }

    if (command == QCmd::Begin) {
    ...
    switch (command) {
        ...
        case QCmd::Abort:
            ...
            *txn_id = INVALID_TXN_ID;
            break;
        case QCmd::UseSnapshotIso:
        case QCmd::UseSerialIso:
            *session_isolation = command == QCmd::UseSerialIso
...
void *client_handler(void *sock_fd) {
    ...
    while (true) {
        ...
        try {
            QCmd fast_command = parse_qcmd(data_recv);
            bool statement_success = false;
            if (ExecuteQCmd(fast_command, &txn_id, context, &session_isolation)) {
