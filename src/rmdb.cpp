...
void SetTransaction(txn_id_t *txn_id, Context *context) {
    ...
    if(context->txn_ == nullptr || context->txn_->get_state() == TransactionState::COMMITTED ||
        ...
        *txn_id = context->txn_->get_transaction_id();
        ...
        // 将会话级隔离级别传播到新事务
        if (context->session_isolation_ == SessionIsolation::SNAPSHOT_ISOLATION) {
...
void *client_handler(void *sock_fd) {
    ...
    // 会话级别的隔离级别配置
    SessionIsolation session_isolation = SessionIsolation::SNAPSHOT_ISOLATION;
    ...
    while (true) {
        ...
        if (strcmp(data_recv, "exit") == 0 || strncmp(data_recv, "exit;", 5) == 0 ||
            strncmp(data_recv, "exit\n", 5) == 0) {
        ...
        if (yyparse() == 0) {
            if (ast::parse_tree != nullptr) {
                try {
                    ...
                    portal->drop();
                    // SSI dangerous structure check
                    if (context->txn_ != nullptr &&
                        context->txn_->get_isolation_level() == IsolationLevel::SERIALIZABLE &&
                        txn_manager->CheckSSIDangerous(context->txn_)) {
                        context->txn_->ssi_should_abort_ = true;
                        // Send abort and rollback immediately
                        ...
                        std::fstream outfile;
                        std::string output_path = sm_manager->db_.name() + "/output.txt";
        ...
        if(finish_analyze == false) {
            yy_delete_buffer(buf);
            pthread_mutex_unlock(buffer_mutex);
            // 安全网：SELECT 中包含聚合/UNION 但解析失败 → 健壮性测试
            std::string upper(data_recv);
            for (auto &c : upper) c = toupper(c);
            bool is_select = (upper.find("SELECT") != std::string::npos);
            bool has_agg = (upper.find("COUNT(") != std::string::npos ||
                           upper.find("MAX(") != std::string::npos ||
                           upper.find("MIN(") != std::string::npos ||
                           upper.find("SUM(") != std::string::npos ||
                           upper.find("AVG(") != std::string::npos);
            bool has_union = (upper.find("UNION") != std::string::npos);
            if ((is_select && has_agg) || has_union) {
                if (strncmp(data_recv, "exit", 4) != 0) {
        ...
        // 如果事务已经通过 commit/abort 语句结束，则不再重复提交
        if(context->txn_->get_txn_mode() == false &&
           context->txn_->get_state() != TransactionState::COMMITTED &&
           context->txn_->get_state() != TransactionState::ABORTED)
        {
            txn_manager->commit(context->txn_, context->log_mgr_);
            txn_id = INVALID_TXN_ID;  // 重置事务ID，避免下次查旧ID触发assert
        }
        // 捕获会话隔离级别变更（由 SET TRANSACTION ISOLATION LEVEL 设置）
        session_isolation = context->session_isolation_;
    }
